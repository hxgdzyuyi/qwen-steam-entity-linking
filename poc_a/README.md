...
* `common/data/steam_games.csv`：共享实体输入，只包含 `canonical_name` 和 `appid` 两列。
* `common/data/steam_games.provenance.csv`：每条数据的分组、来源、来源排名和发布日期，用于检查数据质量。
* `common/data/steam_games.metadata.json`：同步时间、选择规则、数量和上游地址，用于复现实验。
...
python3 common/scripts/sync_steam_dataset.py \
  --popular-count 100 \
  --latest-count 900 \
  --output common/data/steam_games.csv
```

如需生成英文名称快照，可以显式指定：

```bash
python3 common/scripts/sync_steam_dataset.py --language english
```
...
python3 -m pip install --no-cache-dir -r poc_a/requirements-cloud.txt
...
仓库提供 `poc_a/notebooks/runpod_training.ipynb`。Jupyter 会把执行计数和输出写回 Notebook；为保持训练仓库的 Git 状态干净，克隆并固定 commit 后先制作仓库外副本：
...
* Hugging Face dry-run 与公开发布；`HF_REPO_ID` 已直接写为 `hxgdzyuyi/qwen3-8b-steam-entity-linking`，公开写入默认关闭，必须手动把 `PUBLISH_PUBLIC` 改为 `True`。

Notebook 只是下述命令行入口的可视化编排层，所有训练、评测和发布规则仍由同一组 `poc_a/scripts/*.py` 实现。

### 使用独立 Notebook 测试训练好的模型

训练完成后，可以复制并打开专用测试入口：

    cp poc_a/notebooks/runpod_model_testing.ipynb /workspace/runpod_model_testing.ipynb

默认配置会读取 `poc_a/outputs/runpod-full`，并优先加载 `metrics.json` 选出的最佳 checkpoint；尚未运行正式评测时会明确提示并回退到最新 epoch。也可以通过 `CHECKPOINT_EPOCH` 测试指定里程碑，或填写 `HF_ADAPTER_ID` 直接测试已发布的 Hugging Face adapter。
...
python poc_a/scripts/train.py \
  --config poc_a/configs/qwen3_8b_lora.yaml \
  --mode full \
  --resume-from poc_a/outputs/full/checkpoints/checkpoint-<global-step>
```
...
PoC A 的默认目标由 `common/huggingface_repositories.json` 固定为 `hxgdzyuyi/qwen3-8b-steam-entity-linking`。它是 Model Repository，不是 Space。
...
只有在 fork 到其他 Hugging Face namespace 时才需要显式传入 `--repo-id <hf-user>/<model-name>` 覆盖注册目标。
